import { d as setupMockHtml, o as resolveInput, s as resolveServerUrl, t as checkESModule, u as setIsViteDev } from "./utils-DeG18vgO.mjs";
...
const startup = async (argv = [".", "--no-sandbox"], options, customElectronPkg) => {
	...
	for (const root of roots) try {
		electron = createRequire(path.join(root, "package.json"))(electronPackage);
		break;
	} catch (error) {
...
function createElectronPlugin({ prefix, buildConfig, dev, build }) {
	...
	return [{
		...
		apply: "serve",
		config(_, env) {
			if (env.command === "serve") setIsViteDev();
		},
		configResolved(config) {
			isESM = checkESModule(config.root);
			if (!resolveInput(config)) cleanupMock = setupMockHtml(config, false, config.logger);
		},
		...
		configResolved(config) {
			isESM = checkESModule(config.root);
			if (!resolveInput(config)) cleanupMock = setupMockHtml(config, true, config.logger);
		},
		async closeBundle() {
			try {
...
//#endregion
export { triggerStartup as i, defaultPreloadOnstart as n, startup as r, createElectronPlugin as t };
