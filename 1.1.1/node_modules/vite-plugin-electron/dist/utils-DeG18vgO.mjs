...
*/
function setBuildOptions(build, viteVersion = version) {
	if (!build) return;
	const options = getBuildOptions(build);
...
function compatRollupOptions(options, viteVersion = version) {
...
function setupMockHtml(config, isBuild, logger) {
	...
	return async () => {
		if (await fs.promises.readFile(mockFilepath, "utf-8").catch(() => null) === MOCK_INDEX_HTML) await fs.promises.unlink(mockFilepath).catch((err) => {
			logger.warn(`[vite-plugin-electron] Failed to remove mock ${mockFilepath}:`, err);
		});
		if (isBuild && distFilepath) await fs.promises.unlink(distFilepath).catch(() => {});
	};
}
...
//#endregion
export { getIsViteDev as a, resolveViteConfig as c, setupMockHtml as d, toArray as f, createElectronViteDefaults as i, resolveViteConfigBase as l, compatRollupOptions as n, resolveInput as o, withExternalBuiltins as p, createDefaultPreloadConfig as r, resolveServerUrl as s, checkESModule as t, setIsViteDev as u };
