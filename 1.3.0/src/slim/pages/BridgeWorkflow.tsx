...
const PATTERNS: WorkflowPattern[] = [
  {
    label: '机器人已连接',
    icon: Bot,
    test: line => /已连接到 (AstrBot|KouriChat)/.test(line),
    detail: line => `${line.includes('KouriChat') ? 'KouriChat' : 'AstrBot'} · WebSocket 已建立`
  },
  {
    label: '接收微信消息',
    icon: Inbox,
  ...
  },
  {
    ...
    detail: line => `${line.includes('KouriChat') ? 'KouriChat' : 'AstrBot'} · ${groupLabel(line) || 'OneBot 事件'}`
  },
  {
    label: '进入发送队列',
    icon: List,
...
export default function BridgeWorkflow({ logs, ready }: BridgeWorkflowProps) {
