  ...
  private async startUnlocked(): Promise<Result> {
    ...
    try {
      ...
      const proc = (this.options.spawnProcess || spawn)(this.getPython(), [join(dir, 'main.py')], {
        ...
        env: { ...process.env, ASTRWECHAT_VERSION: packageInfo.version, WEFLOW_BRIDGE_CONFIG: this.configStore.getPath(), PYTHONDONTWRITEBYTECODE: '1', PYTHONUNBUFFERED: '1', PYTHONIOENCODING: 'utf-8', PYTHONUTF8: '1' }
      })
      this.proc = proc
  ...
  saveConfig(input: Record<string, unknown>): Promise<Result> {
    return this.exclusive(async () => {
      try {
        ...
        delete next.local_attachment_label
