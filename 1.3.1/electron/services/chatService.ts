import { MESSAGE_DB_UNAVAILABLE } from './messageDbDiagnostics'
...
class ChatService {
   ...
   */
  async getSessions(options?: { skipMessageStats?: boolean }): Promise<{ success: boolean; sessions?: ChatSession[]; error?: string }> {
    try {
      const connectResult = await this.ensureConnected()
      if (!connectResult.success) {
      ...
      // Statistics does not need unread counts; calculating them would scan historical official-account messages.
      if (!options?.skipMessageStats) await this.applySyntheticUnreadCounts(sessions)
  ...
  async getSessionMessageCounts(
    ...
    try {
      ...
      const messageDbSnapshot = await this.getMessageDbCountSnapshot(options?.bypassSessionCache === true)
      if (!messageDbSnapshot.success) {
        errorMessage = messageDbSnapshot.error || MESSAGE_DB_UNAVAILABLE
      ...
      if (pendingSessionIds.length > 0) {
        ...
        if (!tableScanSucceeded) {
          ...
          for (let i = 0; i < pendingSessionIds.length; i += batchSize) {
            ...
            if (!result.success || !result.counts) {
              errorMessage = result.error || '批量读取消息总数失败'
              return { success: false, error: errorMessage }
            }
            const batchCounts = result.counts
  ...
  private async getMessageDbCountSnapshot(forceRefresh = false): Promise<{
    ...
    if (dbPaths.length === 0) {
      this.messageDbCountSnapshotCache = null
      return { success: false, error: MESSAGE_DB_UNAVAILABLE }
    }
  ...
  async getMessageDateCountsBatch(sessionIds: string[], range?: { beginTimestamp: number; endTimestamp: number }): Promise<{ success: boolean; data?: Record<string, Record<string, number>>; error?: string }> {
    try {
      const connectResult = await this.ensureConnected()
      if (!connectResult.success) {
        return { success: false, error: connectResult.error || '数据库未连接' }
      ...
      const messageDbSnapshot = await this.getMessageDbCountSnapshot()
      if (!messageDbSnapshot.success) {
        return { success: false, error: messageDbSnapshot.error || MESSAGE_DB_UNAVAILABLE }
      }
      const result = await wcdbService.getSessionMessageDateCountsBatch(normalizedSessionIds, range)
      if (!result.success || !result.data) {
  ...
  async resolveDownloadedFile(message: Pick<Message, 'fileName' | 'fileMd5' | 'fileSize'>): Promise<string | null> {
    const fileName = basename(String(message.fileName || '').trim())
    const fileMd5 = String(message.fileMd5 || '').trim().toLowerCase()
    const expectedSize = Number(message.fileSize || 0)
    if (!fileName && !/^[a-f0-9]{16,64}$/.test(fileMd5)) return null
    ...
    ].filter((fileRoot) => existsSync(fileRoot))
    if (fileRoots.length === 0) return null
    ...
    const visit = (dir: string, depth: number): void => {
      ...
      try {
        for (const entry of readdirSync(dir, { withFileTypes: true })) {
          ...
          const nameMatches = Boolean(fileName) && lowerName === fileName.toLowerCase()
          const md5Matches = Boolean(fileMd5) && lowerName.includes(fileMd5)
          if (!nameMatches && !md5Matches) continue

          const stat = statSync(fullPath)
          if (expectedSize > 0 && stat.size !== expectedSize) continue
          candidates.push({ path: fullPath, size: stat.size, mtimeMs: stat.mtimeMs })
        }
      } catch {
        // A file can disappear while WeChat is updating its download directory.
      }
    }
