import { inspectMessageDbFiles } from './messageDbDiagnostics'
...
import { queryRangedMessageDateCounts } from './messageDateStats'
...
export class WcdbCore {
  ...
  private lastSecurityStatus: number | null = null
  private lib: any = null
  private koffi: any = null
  private initialized = false
  private initializeInFlight: Promise<boolean> | null = null
  ...
  private formatInitProtectionError(code: number): string {
    const messages: Record<number, string> = {
      ...
      '-2302': 'WCDB 初始化异常，请重试',
      '-2303': 'WCDB 未能成功初始化',
      '-1006': `WCDB 核心初始化失败，原生安全校验未通过${this.lastSecurityStatus === null ? '' : ` (SecurityStatus:${this.lastSecurityStatus})`}。请运行 npm run wcdb:probe，进一步检查正式应用与独立 Worker 的启动环境差异。`,
    }
    const msg = messages[String(code) as unknown as keyof typeof messages]
    return msg ? `${msg} (错误码: ${code})` : `操作失败，错误码: ${code}`
  }

  private writeNativeRuntimeDiagnostics(tag: string): void {
    try {
      const os = require('os') as typeof import('os')
      const env = process.env
      const safe = (value: unknown) => String(value ?? '').replace(/[\r\n]+/g, ' ')
      this.writeLog(
        `[bootstrap] native runtime tag=${tag} thread=${isMainThread ? 'main' : 'worker'} ` +
        `arch=${process.arch}/${os.arch()} platform=${process.platform} osRelease=${safe(os.release())} ` +
        `electron=${safe(process.versions.electron)} node=${safe(process.versions.node)} ` +
        `processorArch=${safe(env.PROCESSOR_ARCHITECTURE)} processorArchW6432=${safe(env.PROCESSOR_ARCHITEW6432)} ` +
        `compatLayer=${safe(env.__COMPAT_LAYER)} electronRunAsNode=${safe(env.ELECTRON_RUN_AS_NODE)} ` +
        `nodeOptions=${safe(env.NODE_OPTIONS)} cwd=${safe(process.cwd())}`,
        true
      )
    } catch (error) {
      this.writeLog(`[bootstrap] native runtime diagnostics failed: ${String(error)}`, true)
    }
  }
  ...
  private async runPostOpenDiagnostics(dbPath: string, dbStoragePath: string | null, sessionDbPath: string | null, wxid: string): Promise<void> {
    try {
      ...
      // Session/contact health is independent of message-shard discovery.
      const messageDbs = await this.listMessageDbs()
      this.writeLog(`[diag:message-db] native success=${messageDbs.success} count=${messageDbs.data?.length ?? -1} error=${messageDbs.error || ''}`, true)
      if (!messageDbs.success || !messageDbs.data?.length) {
        const inventory = inspectMessageDbFiles(dbStoragePath)
        this.writeLog(`[diag:message-db] disk=${JSON.stringify(inventory)}`, true)
        // Bounded, schema-only probes distinguish discovery from query failures.
        // A successful explicit-path query does not repair native discovery/monitoring.
        for (const file of inventory.files.filter(file => file.candidate).slice(0, 3)) {
          const probe = await this.execQuery('message', join(dbStoragePath, file.path),
            "SELECT COUNT(1) AS cnt FROM sqlite_master WHERE type='table' AND lower(name) GLOB 'msg_*'")
          this.writeLog(`[diag:message-db] probe path=${file.path} success=${probe.success} msgTables=${probe.rows?.[0]?.cnt ?? -1} error=${probe.error || ''}`, true)
        }
        await this.dumpDbStatus('message-db-unavailable')
  ...
  private async initializeOnce(): Promise<boolean> {
    ...
    try {
      ...
      try {
        ...
        for (const resPath of resourcePaths) {
          try {
            ...
            if (protectionCode === 0) {
              this.writeLog(`[bootstrap] InitProtection rc=0 path=${resPath}`, true)
              protectionOk = true
              break
            }
      ...
      if (initResult !== 0) {
        ...
        this.writeLog(`[bootstrap] wcdb_init rc=${initResult} dllDir=${dllDir}`, true)
        await this.printLogs(true)
        this.writeNativeRuntimeDiagnostics('wcdb_init_failed')
    ...
    try {
      ...
      if (result === 0 && outPtr[0]) {
        try {
          ...
          const securityMatch = String(jsonStr).match(/SecurityStatus\s*:\s*(-?\d+)/i)
          if (securityMatch) this.lastSecurityStatus = Number(securityMatch[1])
  ...
  async getSessionMessageDateCountsBatch(sessionIds: string[], range?: { beginTimestamp: number; endTimestamp: number }): Promise<{ success: boolean; data?: Record<string, Record<string, number>>; error?: string }> {
    if (!this.ensureReady()) return { success: false, error: 'WCDB 未连接' }
    const normalizedSessionIds = Array.from(new Set((sessionIds || []).map((id) => String(id || '').trim()).filter(Boolean)))
    if (normalizedSessionIds.length === 0) return { success: true, data: {} }
    ...
    if (range !== undefined) {
      try {
        return { success: true, data: await queryRangedMessageDateCounts(this, normalizedSessionIds, range) }
      } catch (error) {
  ...
  async listMessageDbs(): Promise<{ success: boolean; data?: string[]; error?: string }> {
    ...
    try {
      ...
      const data = JSON.parse(jsonStr)
      if (!Array.isArray(data) || data.some(path => typeof path !== 'string' || !path.trim())) {
        return { success: false, error: '消息库列表格式无效' }
      }
      return { success: true, data }
    } catch (e) {
      return { success: false, error: String(e) }
