...
module.exports = async function afterPack(context) {
  if (context.electronPlatformName === 'win32') {
    ...
    if (arch !== 'x64') throw new Error('AstrWeChat host-name adaptation supports Windows x64 only')
    const dllPath = join(context.appOutDir, 'resources', 'resources', 'wcdb', 'win32', arch, 'wcdb_api.dll')
    const changed = require('./wcdb-host-branding.cjs').adaptFile(dllPath)
    console.log(`[afterPack] AstrWeChat WCDB host-name adaptation ${changed ? 'applied' : 'verified'}`)
    require('./verify-package.cjs').verifyWindowsPackage(context.appOutDir, arch)
    return
  }
