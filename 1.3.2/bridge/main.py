...
def emit_status():
    emit("status", {
        ...
        "ob_url": cfg.OB_URL,
        "bot_backend": cfg.BOT_BACKEND,
        "ob_state": state.ob_state,
        "ob_error": state.ob_error,
    })
    ...
    with state.run_lock:
        ...
        if previous_client and previous_client.is_alive():
            ...
            if previous_client.is_alive():
                emit_log("上一条机器人连接仍在退出，请稍后重试", "error")
                return

        lock_identity = f"astrwechat|{state._self_id_int}"
        if not acquire_instance_lock(lock_identity):
            emit_log("另一个 AstrWeChat Bridge 正在使用同一微信身份", "error")
            return
...
def _stop_bridge():
    ...
    with state.bridge_lock:
        if state.bridge_instance:
            state.bridge_instance.close()
    ...
    if ws and loop and loop.is_running():
        ...
        try:
            ...
            emit_log(f"关闭机器人连接时出现异常: {e}", "warn")
...
def _bridge_loop():
    ...
    emit_log(f"Bridge 启动 | 服务端: {cfg.OB_LABEL}")
...
def _apply_config(new_cfg: dict):
    """Electron owns persistence; an old queued update must never overwrite a newer saved file."""
    from connection_config import resolve_connection
    connection = resolve_connection(new_cfg)
    if connection != (cfg.BOT_BACKEND, cfg.OB_LABEL, cfg.OB_URL, cfg.OB_TOKEN):
        emit_log("连接参数变更需要重启 Bridge，已拒绝热切换", "error")
        return
    direct_window = new_cfg.get("uia_direct_window", False) is True
    if direct_window and new_cfg.get("group_reply_filter_mode") != "whitelist":
        emit_log("独立窗口发送模式必须使用消息过滤白名单，已拒绝配置更新", "error")
        return
    if direct_window != cfg.UIA_DIRECT_WINDOW:
        emit_log("发送模式变更需要重启 Bridge，已拒绝热切换", "error")
        return
    cfg.config = new_cfg
    ...
    cfg.UIA_DETAILED_LOGGING = bool(new_cfg.get("uia_detailed_logging", cfg.UIA_DETAILED_LOGGING))
    if state.sender_instance is not None:
        state.sender_instance.set_detailed_logging(cfg.UIA_DETAILED_LOGGING)
    ...
    try:
        ...
        pass
    try:
        cfg.ACTIVE_REPLY_CONTEXT_LINES = min(100, max(0, int(new_cfg.get("active_reply_context_lines", cfg.ACTIVE_REPLY_CONTEXT_LINES))))
    except (TypeError, ValueError):
        pass
    ...
    (
        cfg.GROUP_REPLY_FILTER_MODE,
        cfg.GROUP_REPLY_FILTER_SESSIONS,
    ) = cfg._group_filter_from_config(new_cfg)
    cfg.ASTRBOT_ATTACHMENTS = new_cfg.get("astrbot_attachments", cfg.ASTRBOT_ATTACHMENTS)
    emit_log("配置已更新")
