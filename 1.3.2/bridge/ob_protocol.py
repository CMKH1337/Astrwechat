...
def _send_with_permission(method, contact, payload, can_send=None):
    # Media preparation can take minutes; recheck permissions in the send worker.
    if can_send is not None and not can_send():
        log.warning("[OB11] 会话权限已变更，取消发送: %s", contact)
        return False
    return method(contact, payload)


async def _send_image_operation(contact: str, file_val: str, can_send=None) -> bool:
    img_path = None
    ...
    try:
        sent = await asyncio.to_thread(_send_with_permission, state.sender_instance.send_image, contact, img_path, can_send)
        if sent:
...
async def _send_file_operation(contact: str, file_val: str, media_label: str = "File", can_send=None) -> bool:
    temporary_dir = None
    ...
    try:
        sent = await asyncio.to_thread(_send_with_permission, state.sender_instance.send_file, contact, file_path, can_send)
        if sent:
...
async def _process_send_request(request: dict):
    ...
    def can_send():
        return (config.is_direct_window_session_allowed(session_id)
                and (not is_group or config.is_group_reply_allowed(session_id)))

    if not can_send():
        log.info(
            f"[OB11] Group reply blocked by session filter: "
            f"action={action} session={session_id or '-'} "
    ...
    if action in ("upload_private_file", "upload_group_file"):
        ...
        if not file_val:
            ...
            return
        await _send_file_operation(contact, file_val, can_send=can_send)
        return
    ...
    for operation_type, value in operations:
        if not can_send():
            log.warning("[OB11] 会话白名单已变更，取消剩余发送: %s", session_id)
            return
        if operation_type == "text":
            sent = await asyncio.to_thread(_send_with_permission, state.sender_instance.send_text, contact, value, can_send)
            if sent:
                log.info(f"[OB11] 文字已发送至 {contact}: {value[:50]}")
            else:
                log.error(f"[OB11] 文字发送失败: {contact}: {value[:50]}")
        elif operation_type == "image":
            await _send_image_operation(contact, value, can_send=can_send)
        elif operation_type == "file":
            await _send_file_operation(contact, value, can_send=can_send)
            ...
            await _send_file_operation(contact, value, media_label="Video", can_send=can_send)
...
def _resolve_reply_contact(action: str, params: dict):
    ...
    if is_group:
        ...
        contact = (
            private_session_id
            if config.SEARCH_BY_WXID and not config.UIA_DIRECT_WINDOW and private_session_id
...
async def _handle_ob_api(data: dict):
    ...
    if action in ("send_msg", "send_private_msg", "send_group_msg"):
        is_group, target_id, contact, session_id = _resolve_reply_contact(
            action, params
        )
