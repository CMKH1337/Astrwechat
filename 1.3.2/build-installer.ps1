...
try {
    ...
    Write-Step 'Checking packaging configuration'
    Invoke-Npm @('run', 'test:packaging')

    $previousSizes = @{}
    Get-ChildItem -LiteralPath $releasePath -Filter '*Setup.exe' -File -ErrorAction SilentlyContinue |
        ForEach-Object { $previousSizes[$_.Name] = $_.Length }
    ...
    foreach ($installer in $installers) {
        Write-Host ("Installer: {0} ({1:N1} MiB)" -f $installer.FullName, ($installer.Length / 1MB)) -ForegroundColor Green
        if ($previousSizes.ContainsKey($installer.Name) -and $previousSizes[$installer.Name] -gt 0) {
            $previousSize = $previousSizes[$installer.Name]
            $saved = $previousSize - $installer.Length
            Write-Host ("Previous: {0:N1} MiB; reduction: {1:N1} MiB ({2:N1}%)" -f ($previousSize / 1MB), ($saved / 1MB), (100.0 * $saved / $previousSize)) -ForegroundColor DarkGray
        }
    }
