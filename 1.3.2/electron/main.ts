...
import { BridgeWechatSource } from './services/bridgeWechatSource'
...
// Windows x64 packages adapt the WCDB host-name constants in after-pack.cjs.
// Keep other platform identities unchanged until their native runtimes are verified.
app.setName(process.platform === 'win32' && process.arch === 'x64' ? 'AstrWeChat' : 'WeFlow')
// Preserve existing settings/data; directory migration is separate from EXE branding.
try {
...
const bridgeWechatSource = new BridgeWechatSource({
  getSettings: () => {
    const cfg = configService || new ConfigService()
    return {
      host: String(cfg.get('httpApiHost') || '127.0.0.1').trim() || '127.0.0.1',
      port: Number(cfg.get('httpApiPort')) || 5031,
      token: String(cfg.get('httpApiToken') || ''),
      wxid: cfg.getMyWxidCleaned()
    }
  },
  isDatabaseConnected: () => wcdbService.isConnected(),
  getDatabaseActivity: () => wcdbService.getPendingOperationSummary(),
  saveToken: token => (configService || new ConfigService()).set('httpApiToken', token),
  enableApi: () => (configService || new ConfigService()).set('httpApiEnabled', true),
  enablePush: () => (configService || new ConfigService()).set('messagePushEnabled', true),
  warmupPush: () => messagePushService.handleConfigChanged('messagePushEnabled'),
  http: httpService
})

const bridgeManager = new BridgeManager({
  getConfigDir: () => join(app.getPath('userData'), 'bridge'),
  getManagedConfig: () => ({
    ...bridgeWechatSource.getConfig(),
    uia_detailed_logging: Boolean((configService || new ConfigService()).get('uiaDetailedLogging'))
  }),
  prepareStart: context => bridgeWechatSource.prepareStart(context),
  getBridgeDir: () => {
    const candidates = [
  ...
  onStatus: status => BrowserWindow.getAllWindows().forEach(w => {
    if (!w.isDestroyed()) w.webContents.send('bridge:status', status)
  }),
  onLog: line => BrowserWindow.getAllWindows().forEach(w => {
    if (!w.isDestroyed()) w.webContents.send('bridge:log', line)
  })
})
...
function registerIpcHandlers() {
  ...
  ipcMain.handle('config:set', async (_, key: string, value: any) => {
    ...
    if ((key === 'httpApiToken' || key === 'uiaDetailedLogging') && bridgeManager.isRunning()) {
      const synced = await bridgeManager.saveConfig({})
      if (!synced.success || synced.warning) throw new Error(synced.error || synced.warning)
    }
    return result
  })
  ...
  ipcMain.handle('config:clear', async () => {
    ...
    try {
      await bridgeManager.resetConfig()
    } catch (error) {
  ...
  ipcMain.handle('wcdb:open', async (_, dbPath: string, hexKey: string, wxid: string) => {
    ...
    try {
      // Never let a Bridge with the old self ID continue sending while changing databases.
      const bridgeStopped = bridgeManager.isRunning()
      if (bridgeStopped && !await bridgeManager.stop()) {
        return { success: false, error: '旧 Bridge 尚未退出，请先停止 Bridge 再切换微信连接' }
      }
      ...
      if (success) {
        ...
        return { success: true, bridgeStopped }
      }
  ...
  ipcMain.handle('wcdb:close', async () => {
    if (bridgeManager.isRunning() && !await bridgeManager.stop()) throw new Error('请先停止 Bridge 再断开微信连接')
  ...
  ipcMain.handle('chat:getSessions', async (_, options?: { skipMessageStats?: boolean }) => {
    return chatService.getSessions(options)
  })
  ...
  ipcMain.handle('chat:getMessageDateCountsBatch', async (_, sessionIds: string[], range?: { beginTimestamp: number; endTimestamp: number }) => {
    return chatService.getMessageDateCountsBatch(sessionIds, range)
  })
  ...
  ipcMain.handle('http:start', async (_, port?: number, host?: string) => {
    ...
    const result = await httpService.start(port || 5031, bindHost)
    if (result.success && bridgeManager.isRunning()) {
      const synced = await bridgeManager.saveConfig({})
      if (!synced.success || synced.warning) return { success: false, error: synced.error || synced.warning }
    }
    return result
  })
  ...
  ipcMain.handle('bridge:getConfig', async () => {
    try {
      return { success: true, config: await bridgeManager.getConfig(), warning: bridgeManager.configWarning }
    } catch (e) {
      return { success: false, error: String(e) }
    }
  })
  ...
  ipcMain.handle('bridge:saveConfig', async (_, cfg: Record<string, unknown>) => {
    return bridgeManager.saveConfig(cfg)
  })
}
