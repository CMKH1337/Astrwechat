...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  chat: {
    ...
    getSessions: (options?: { skipMessageStats?: boolean }) => ipcRenderer.invoke('chat:getSessions', options),
    ...
    getMessageDateCountsBatch: (sessionIds: string[], range?: { beginTimestamp: number; endTimestamp: number }) => ipcRenderer.invoke('chat:getMessageDateCountsBatch', sessionIds, range),
  ...
  bridge: {
    ...
    saveConfig: (cfg: Record<string, unknown>) => ipcRenderer.invoke('bridge:saveConfig', cfg),
    ...
    onStatus: (callback: (status: Record<string, unknown>) => void) => {
      const listener = (_: unknown, status: Record<string, unknown>) => callback(status)
