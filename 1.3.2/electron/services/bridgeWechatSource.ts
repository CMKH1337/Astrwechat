...
interface Options {
  ...
  getDatabaseActivity?: () => { count: number; oldestType?: string; oldestAgeMs: number }
...
export class BridgeWechatSource {
  ...
  async prepareStart(context?: BridgeStartContext): Promise<void> {
    ...
    const activity = this.options.getDatabaseActivity?.()
    if (activity && activity.oldestAgeMs >= 5000) {
      context?.reportWarning(`微信数据库已连接，但后台请求仍在执行：${activity.oldestType || 'unknown'}，已等待 ${Math.floor(activity.oldestAgeMs / 1000)} 秒。Bridge 继续启动，消息读取可能延迟；若持续不恢复，请重启软件。`)
    }
    const settings = this.options.getSettings()
