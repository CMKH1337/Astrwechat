...
class MessagePushService {
  ...
  async handleConfigChanged(key: string): Promise<void> {
    ...
    // The connection form persists dbPath/decryptKey/myWxid immediately after
    // a successful wcdb:open. That open reports through handleDatabaseConnected
    // and already supplies the active runtime connection. Do not close or
    // reconnect WCDB while saving those fields: its native shutdown followed by
    // a same-process re-initialization can return -1006.
  ...
  async handleDatabaseConnected(dbPath: string, decryptKey: string, myWxid: string): Promise<void> {
    chatService.setRuntimeConfig({ dbPath, decryptKey, myWxid })
    await this.refreshConfiguration('database-connected')
  }
  ...
  private async bootstrapBaseline(): Promise<void> {
    const sessionsResult = await chatService.getSessions({ skipMessageStats: true })
  ...
  private async pushSessionMessages(
    ...
    if (!newMessagesResult.success) {
      console.warn('[MessagePushService] Read new messages failed:', newMessagesResult.error || 'unknown')
    }
  ...
  private async buildPayload(session: ChatSession, message: Message): Promise<MessagePushPayload | null> {
    ...
    if (isGroup) {
      ...
      const groupName = sessionDisplayName(sessionId, groupInfo?.displayName, session.displayName)
    ...
    return {
      ...
      messageKey,
      avatarUrl,
      sourceName: sessionDisplayName(sessionId, contactInfo?.displayName, session.displayName),
      senderUsername: message.senderUsername || undefined,
      content,
      appMsgKind,
      fileName: inferredFileName,
  ...
  private async normalizePushAvatarUrl(avatarUrl?: string): Promise<string | undefined> {
    ...
    try {
      ...
      const extension = this.getImageExtFromMime(mimeType)
      const hash = createHash('sha1').update(normalized).digest('hex')
      const filePath = path.join(this.pushAvatarCacheDir, `avatar_${hash}.${extension}`)
