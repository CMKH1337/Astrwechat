...
export class WcdbService {
  ...
  private messageId = 0
  private pending = new Map<number, { resolve: (val: any) => void; reject: (err: any) => void; type: string; startedAt: number }>()
  private confirmedConnected = false
  ...
  private initWorker() {
    ...
    try {
      ...
      worker.on('message', (msg: any) => {
        ...
        if (p) {
          // The worker reports the actual handle state with completed operations.
          // This is a lifecycle snapshot, not a claim that native queries are fast.
          if (typeof msg.connected === 'boolean') {
            this.confirmedConnected = msg.connected
            if (!msg.connected) this.activeConnection = null
          }
      ...
      worker.on('exit', (code) => {
        ...
        this.activeConnection = null
        this.confirmedConnected = false
        this.rejectPending(new Error(`WCDB Worker 已退出 (退出码: ${code})`))
      })
  ...
  private runLifecycleExclusive<T>(task: () => Promise<T>): Promise<T> {
    // Invalidate the public snapshot immediately, even while close/switch is queued.
    this.pendingLifecycle += 1
    const operation = this.lifecycleChain.then(task)
    const tracked = operation.finally(() => { this.pendingLifecycle -= 1 })
    this.lifecycleChain = tracked.then(() => undefined, () => undefined)
    return tracked
  }

  async initialize(): Promise<{ success: boolean; error?: string }> {
    return this.runLifecycleExclusive(() => this.callWorker<{ success: boolean; error?: string }>('initialize'))
  }
  ...
  async testConnection(accountDir: string, hexKey: string): Promise<{ success: boolean; error?: string; sessionCount?: number }> {
    return this.runLifecycleExclusive(() => this.callWorker('testConnection', { accountDir, hexKey }))
  }
  ...
  async open(accountDir: string, hexKey: string): Promise<boolean> {
    const alreadyOpen = () => this.worker !== null && this.confirmedConnected &&
      this.activeConnection?.accountDir === accountDir && this.activeConnection?.hexKey === hexKey
    // Match the native same-account fast path without enqueueing behind a slow query.
    // Never bypass a queued close, test or account switch.
    if (this.pendingLifecycle === 0 && alreadyOpen()) return true
  ...
  /**
   * 获取数据库连接状态
   * Worker 已确认的句柄状态，不再把纯状态检查排在同步原生查询之后。
   * 不代表查询响应速度；生命周期操作、退出或错误期间立即返回未连接。
   */
  async isConnected(): Promise<boolean> {
    return this.worker !== null && this.confirmedConnected && this.pendingLifecycle === 0
  }

  getPendingOperationSummary(): { count: number; oldestType?: string; oldestAgeMs: number } {
    let oldest: { type: string; startedAt: number } | undefined
    for (const item of this.pending.values()) {
      if (!oldest || item.startedAt < oldest.startedAt) oldest = item
    }
    return { count: this.pending.size, oldestType: oldest?.type, oldestAgeMs: oldest ? Math.max(0, Date.now() - oldest.startedAt) : 0 }
  }
  ...
  async getSessionMessageDateCountsBatch(sessionIds: string[], range?: { beginTimestamp: number; endTimestamp: number }): Promise<{ success: boolean; data?: Record<string, Record<string, number>>; error?: string }> {
    return this.callWorker('getSessionMessageDateCountsBatch', { sessionIds, range })
  }
