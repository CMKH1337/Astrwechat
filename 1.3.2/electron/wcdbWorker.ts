        ...
        try {
            ...
            switch (type) {
                ...
                case 'initialize':
                    ...
                    result = { success, error: success ? undefined : core.getLastInitError() }
                    break
                    }
                case 'setMonitor':
                    {
                ...
                case 'getSessionMessageDateCountsBatch':
                    result = await core.getSessionMessageDateCountsBatch(payload.sessionIds, payload.range)
                    break
            ...
            parentPort!.postMessage({ id, result, connected: core.isConnected() })
        } catch (e) {
            parentPort!.postMessage({ id, error: String(e), connected: core.isConnected() })
        }
    })
}
