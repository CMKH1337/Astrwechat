...
; Windows x64 主程序为 AstrWeChat.exe，安装根目录保留 astrwechat；WCDB 名称适配由 afterPack 验证。
!ifdef APP_FILENAME
  !undef APP_FILENAME
!endif
...
!macro customInstall
  ...
  ; 明确使用安装目录中的 icon.ico 创建桌面快捷方式，避免旧图标缓存。
  ; 使用构建器提供的 APP_EXECUTABLE_FILENAME，目标为 AstrWeChat.exe。
