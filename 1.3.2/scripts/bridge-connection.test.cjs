...
test('message push baseline and subsequent refresh use lightweight sessions, preserving native unread counts', async () => {
  ...
  const mocks = {
    ...
    '../../shared/session-display-name': require('../shared/session-display-name.ts')
  }
...
test('local administrator edits persist and hot-apply without restarting the connection', async t => {
  const f = fixture(t)
  assert.equal((await f.manager.start()).success, true)
  const admin = { uid: 'AW-0123456789ABCDEF01234567', note: 'test owner' }
  const result = await f.manager.saveConfig({ local_command_admins: [admin], local_attachment_label: '附件编号', ac_cache_hours: 12 })
  assert.equal(result.success, true)
  assert.equal(f.children.length, 1)
  const command = f.children[0].commands.at(-1)
  assert.equal(command.cmd, 'update_config')
  assert.deepEqual(command.config.local_command_admins, [admin])
  assert.equal(Object.hasOwn(command.config, 'local_attachment_label'), false)
  assert.equal(command.config.ac_cache_hours, 12)
  assert.equal(f.children[0].spawnOptions.env.ASTRWECHAT_VERSION, require('../package.json').version)
  const saved = JSON.parse(fs.readFileSync(f.children[0].spawnOptions.env.WEFLOW_BRIDGE_CONFIG, 'utf8'))
  assert.deepEqual(saved.local_command_admins, [admin])
  assert.equal(Object.hasOwn(saved, 'local_attachment_label'), false)
})

test('direct-window mode requires whitelist at validation, save and start', async t => {
  assert.equal(defaults.uia_direct_window, false)
  assert.match(validateBridgeConnection({ ...defaults, uia_direct_window: true }), /白名单/)
  assert.equal(validateBridgeConnection({ ...defaults, uia_direct_window: true, group_reply_filter_mode: 'whitelist' }), null)
  const f = fixture(t)
  assert.equal((await f.manager.saveConfig({ uia_direct_window: true })).success, false)
  assert.notEqual((await f.manager.getConfig()).uia_direct_window, true)
  const invalid = fixture(t, { uia_direct_window: true })
  assert.equal((await invalid.manager.start()).success, false)
  assert.equal(invalid.children.length, 0)
})

test('direct-window mode switch restarts and enabled mode cannot save blacklist', async t => {
  assert.equal(requiresBridgeRestart(defaults, { ...defaults, uia_direct_window: true }), true)
  assert.equal(requiresBridgeRestart({ ...defaults, uia_direct_window: true }, defaults), true)
  const f = fixture(t)
  assert.equal((await f.manager.start()).success, true)
  const result = await f.manager.saveConfig({ uia_direct_window: true, group_reply_filter_mode: 'whitelist', group_reply_filter_sessions: ['g@chatroom'] })
  assert.equal(result.success, true)
  assert.equal(result.restarted, true)
  assert.equal(f.children.length, 2)
  assert.equal(f.maxAlive(), 1)
  assert.equal((await f.manager.saveConfig({ group_reply_filter_mode: 'blacklist' })).success, false)
  assert.equal((await f.manager.getConfig()).group_reply_filter_mode, 'whitelist')
  assert.equal((await f.manager.saveConfig({ uia_direct_window: false, group_reply_filter_mode: 'blacklist' })).success, true)
})
