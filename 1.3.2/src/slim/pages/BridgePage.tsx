...
import { buildSessionOptions, type SessionOption } from './bridgeSessionOptions'
...
const normalizeLoadedConfig = (raw: Record<string, unknown>) => {
  ...
  return {
    ...
    config: {
      ...
      uia_direct_window: raw.uia_direct_window === true,
...
export default function BridgePage() {
  ...
  const [scanMessage, setScanMessage] = useState('尚未扫描会话')
  const [scanErrorDetail, setScanErrorDetail] = useState('')
  const bottomRef = useRef<HTMLDivElement>(null)
  const pickerRef = useRef<HTMLDivElement>(null)
  const scanningRef = useRef(false)
  const sessionOptionsRef = useRef<SessionOption[]>([])
  ...
  const scanSessions = async (force = false) => {
    if (scanningRef.current) return
    const now = Date.now()
    if (
      !force
      && sessionOptionsRef.current.length > 0
    ...
    try {
      ...
      const publishOptions = (contacts: Record<string, { displayName?: string }> = {}) => {
        const options = buildSessionOptions(sessions, contacts)
        sessionOptionsRef.current = options
        setSessionOptions(options)
        const groups = options.filter(option => option.kind === 'group').length
        setScanMessage(`已扫描 ${options.length} 个会话：${groups} 个群聊，${options.length - groups} 个私聊/其他会话`)
      }
      // Show selectable IDs immediately; contact lookup failure must not hide private chats.
      publishOptions()
      lastScanAtRef.current = Date.now()
      setScanState('success')
      const ids = [...new Set(sessions.map(session => String(session.username || '').trim()).filter(Boolean))]
      if (ids.length > 0) {
        let timeout: ReturnType<typeof setTimeout> | undefined
        try {
          const names = await Promise.race([
            window.electronAPI.chat.enrichSessionsContactInfo(ids),
            new Promise<never>((_, reject) => {
              timeout = setTimeout(() => reject(new Error('会话名称补全超时，仍可按会话 ID 选择')), 8000)
            }),
          ])
          if (names.success) publishOptions(names.contacts || {})
          else setScanErrorDetail('会话名称补全失败，仍可按会话 ID 选择')
        } catch {
          setScanErrorDetail('会话名称补全失败或超时，仍可按会话 ID 选择')
        } finally {
  ...
  useEffect(() => {
    ...
    void scanSessions()
    const timer = setInterval(() => {
      if (Date.now() - lastScanAtRef.current >= 30_000) void scanSessions()
    }, 10_000)
    const refreshTimer = setTimeout(() => void scanSessions(), remaining)
    return () => {
      clearInterval(timer)
  ...
  const duplicateSessionNames = useMemo(() => {
    const counts = new Map<string, number>()
    for (const group of sessionOptions) {
  ...
  const sessionOptionLabel = (group: SessionOption) =>
    duplicateSessionNames.has(group.name)
      ? `${group.name} · ${shortSessionId(group.id)}`
      : group.name

  const filteredSessionOptions = useMemo(() => {
    const keyword = sessionSearch.trim().toLocaleLowerCase()
    if (!keyword) return sessionOptions
    return sessionOptions.filter(group =>
  ...
  const selectedOptionMap = useMemo(
    () => new Map(sessionOptions.map(group => [group.id, group])),
    [sessionOptions]
  )
  ...
  const manualUnknownEntries = manualEntries.filter(
    entry => !selectedOptionMap.has(entry)
  )
  ...
  const toggleSessionOption = (group: SessionOption) => {
    setConfig(prev => {
      ...
      <div className={`bridge-overview-grid ${tab === 'overview' ? '' : 'is-hidden'}`}>
          ...
          <section className="slim-card bridge-overview-card">
            ...
            <div className="bridge-summary-list">
              ...
              <div><span>扫描会话</span><strong>{sessionOptions.length}</strong></div>
...
token = "与此处填写的 Token 一致"`}</pre>
          ...
          <div className="slim-card bridge-filter-card">
            ...
            <div className="bridge-filter-toolbar">
              <div className="bridge-filter-segmented" role="group" aria-label="会话过滤模式">
                ...
                <button
                  ...
                  disabled={config.uia_direct_window}
                  title={config.uia_direct_window ? '请先关闭独立窗口发送模式' : undefined}
              ...
              {!config.uia_direct_window && (
                <span className="slim-field__inline-hint">搜索发送模式仅过滤群聊；私聊白名单限制在独立窗口发送模式下生效。</span>
              )}
            </div>
            ...
            <div className="bridge-filter-scan-row">
              ...
              <button
                type="button"
                className="slim-btn slim-btn--secondary"
                onClick={() => void scanSessions(true)}
            ...
            {pickerOpen && (
              <div ref={pickerRef} className="bridge-filter-picker">
                ...
                <div className="bridge-filter-picker__list" role="listbox" aria-multiselectable="true">
                  {filteredSessionOptions.map(group => {
                    ...
                    return (
                      <button
                        ...
                        title={group.id}
                        onClick={() => toggleSessionOption(group)}
                      >
                        <span className="bridge-filter-check">{selected && <Check size={14} />}</span>
                        <span className="bridge-filter-option__kind">{group.kind === 'group' ? '群聊' : '私聊/其他'}</span>
                        <span className="bridge-filter-option__name">{sessionOptionLabel(group)}</span>
                      </button>
                    )
                  })}
                  {filteredSessionOptions.length === 0 && (
                    <div className="bridge-filter-picker__empty">
                      {sessionOptions.length === 0 ? '暂无已扫描的会话' : '没有匹配的会话'}
                    </div>
                  )}
                </div>
                <div className="bridge-filter-picker__footer">
                  {filteredSessionOptions.length} / {sessionOptions.length} 个会话
                </div>
              </div>
            )}
                    ...
                    {selectedScanned.map(sessionId => {
                      ...
                      return (
                        <span key={sessionId} className="bridge-filter-chip" title={sessionId}>
                          <span>{sessionOptionLabel(group)}</span>
                          <button type="button" aria-label={`移除 ${sessionOptionLabel(group)}`} onClick={() => removeFilterSession(sessionId)}>
                            <X size={12} />
                          </button>
                        </span>
                      )
            ...
            <div className="bridge-filter-manual">
              ...
              {manualUnknownEntries.length > 0 && (
                <div className="bridge-filter-manual__warning">
                  <CircleAlert size={13} />
                  以下 ID 暂未扫描到，请确认填写的是真实微信会话 ID：{manualUnknownEntries.join('、')}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="slim-card">
            ...
            <div className="slim-field">
              <label htmlFor="uia-direct-window">独立窗口发送
                <span>直接激活已打开的目标聊天窗口，不使用搜索；切换后保存会重启 Bridge</span>
              </label>
              <label className="slim-toggle">
                <input
                  id="uia-direct-window"
                  type="checkbox"
                  checked={config.uia_direct_window}
                  disabled={!config.uia_direct_window && config.group_reply_filter_mode !== 'whitelist'}
                  onChange={event => setConfig(prev => ({ ...prev, uia_direct_window: event.target.checked }))}
                />
                <span className="slim-toggle__track" />
              </label>
            </div>
            {config.group_reply_filter_mode !== 'whitelist' && (
              <p className="slim-field__inline-hint">请先将消息过滤切换为白名单模式，再开启独立窗口发送。</p>
            )}
            {config.uia_direct_window && (
              <div className="bridge-config-warning" role="alert">
                请手动将白名单中选择的每个会话打开为独立聊天窗口，并保持窗口存在，可最小化。
                发送时会将目标窗口置前，请保持聊天输入框可输入，避免同时操作微信。
                未打开、标题不匹配或存在同名窗口时将停止发送，不会回退搜索。
                此模式仅允许白名单内会话发送，未列入白名单的私聊也不会回复。
                {config.group_reply_filter_sessions.length === 0 && ' 当前白名单为空，不会发送任何消息。'}
              </div>
            )}
            <div className="slim-field">
