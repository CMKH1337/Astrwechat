...
export interface ElectronAPI {
  ...
  config: {
    ...
    clear: () => Promise<{ success: boolean; errors?: string[] }>
  }
  auth: {
  ...
  chat: {
    ...
    getSessions: (options?: { skipMessageStats?: boolean }) => Promise<{ success: boolean; sessions?: ChatSession[]; error?: string }>
    ...
    getMessageDateCountsBatch: (sessionIds: string[], range?: { beginTimestamp: number; endTimestamp: number }) => Promise<{ success: boolean; data?: Record<string, Record<string, number>>; error?: string }>
  ...
  bridge: {
    ...
    stop: () => Promise<{ success: boolean }>
    status: () => Promise<{ starting?: boolean; startup_stage?: string; running: boolean; paused: boolean; ob_connected: boolean; weflow_url?: string; ob_url?: string; bot_backend?: 'astrbot' | 'kourichat'; ob_state?: string; ob_error?: string; processRunning: boolean }>
    getLogs: () => Promise<string[]>
    clearLogs: () => Promise<{ success: boolean }>
    getConfig: () => Promise<{ success: boolean; config?: Record<string, unknown>; error?: string; warning?: string }>
    saveConfig: (cfg: Record<string, unknown>) => Promise<{ success: boolean; error?: string; warning?: string; restarted?: boolean; config?: Record<string, unknown> }>
