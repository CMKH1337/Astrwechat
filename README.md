...
> [!IMPORTANT]
> AstrWeChat 不是微信官方产品，也不是 AstrBot 官方组件。使用前请自行确认当地法律法规、软件协议和数据授权要求。

## 下载

前往 [GitHub Releases](https://github.com/CMKH1337/Astrwechat/releases/latest) 下载最新版本。
...
- Windows 10 或更高版本
- 已安装并登录微信客户端
- 当前 Windows 用户可以访问微信本地数据目录
- 如需通过 Bridge 发送消息，需要安装 Python 3.10 或更高版本
...
2. 保持微信客户端已登录，点击“自动获取”读取数据库密钥。
3. 如需解密聊天图片，可使用“自动获取图片密钥”或“内存扫描 AES”。
...
完整接口文档见 [docs/HTTP-API.md](docs/HTTP-API.md)。
...
AstrWeChat 并非从零开始，项目开发参考或使用了以下项目的代码与设计思路：
...
请以仓库中的 [LICENSE](LICENSE) 文件及各第三方依赖、原生库和资源自身的许可证声明为准。基于或修改本项目进行分发时，请同时遵守上游项目的署名、非商业和相同方式共享等要求（如适用）。

---

如果 AstrWeChat 对你有帮助，欢迎提交 Issue、改进文档或参与开发。
